...
const FormEditApoderado = ({
  ...
  let docSize = contratoPartial.documentos.length;

  useEffect(() => {
    let apoderado_ubigeo = contratoPartial?.parentescos?.[0]?.apoderado?.ubigeo;
    if (apoderado_ubigeo.length !== 6) {
      apoderado_ubigeo = "010101";
    }
    setDocDep(apoderado_ubigeo.substring(0, 2));
    setDocProv(apoderado_ubigeo.substring(0, 4));
    setDocDis(apoderado_ubigeo);

    setProvincias(
      Region.instance(apoderado_ubigeo.substring(0, 2)).getProvincies()
    );
    setDistritos(
      Province.instance(apoderado_ubigeo.substring(0, 4)).getDistricts()
    );
  }, []);
  ...
  const getIndexOfId = (id: any) => {
    const index = getValues().documentos.findIndex((item) => item.id === id);
    if (index >= 0) {
      return index;
    } else {
      docSize = docSize + 1;
        ...
        <Col>
          <div key="inline-radio">
            <Form.Check
              ...
              type="radio"
              value={"true"}
              defaultChecked={contratoPartial.firma_contrato ? true : false}
            ...
            <Form.Check
              ...
              type="radio"
              value={"false"}
              defaultChecked={contratoPartial.firma_contrato ? false : true}
              {...register("firma_contrato", { required: "*Requerido" })}
            />
          </div>
                ...
                return (
                  <div key={index}>
                    <Form.Check
                      ...
                      type="checkbox"
                      defaultChecked={
                        contratoPartial.documentos.find(
                          (item) => documento.id === item.id
                        )?.entregado
                      }
                      onChange={({ target }) => {
                        const newIndex = getIndexOfId(documento.id);
                        setValue(`documentos.${newIndex}.id`, documento.id, {
                          shouldDirty: true,
                        });
                        setValue(
                          `documentos.${newIndex}.nombre_documento`,
                          documento.tipo_documento,
                          { shouldDirty: true }
                        );
                        setValue(
                          `documentos.${newIndex}.entregado`,
                          target.checked,
                          { shouldDirty: true }
                        );
                      }}
