...
const FormEstudiante = ({
  ...
  onSubmit: SubmitHandler<{ estudiante: EstudianteGuardado }>;
}) => {
  ...
  useEffect(() => {
    if (contratoPartial?.estudiante?.ubigeo != "") {
      setDocDep(contratoPartial?.estudiante.ubigeo.substring(0, 2));
      setDocProv(contratoPartial?.estudiante.ubigeo.substring(0, 4));
      setDocDis(contratoPartial?.estudiante.ubigeo);
        ...
        <Form.Group as={Col} lg="4" controlId="_conEstUbi">
          ...
          <Form.Control
            ...
            {...register("estudiante.ubigeo", {
              required:
                "*Requerido: Seleccione Departamento/Provincia/Distrito",
