...
const AgregarEstudiantes = () => {
  ...
  const { clase, estudiantes } = useRouteLoaderData(
    "clase_estudiantes"
  ...
  const estudiantes_selectos = estudiantes_grado.filter(
    (estudiantes) => !clase.estudiantes.some((tmp) => tmp.id === estudiantes.id)
  );
      ...
      <Form id="from_añadir_estudiante" onSubmit={(e) => onSubmit(e)}>
        <ModalHeader className="modal-header-custom">
          <div className="d-flex justify-content-between align-items-center">
            <h6 className="mb-0">
              AÑADIR ESTUDIANTES A LA CLASE DE{" "}
                ...
                {estudiantes_selectos.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={2} className="sin-resultados-mensaje">
                      NO HAY MÁS ESTUDIANTES PARA INSCRIBIR
                    </td>
                  </tr>
                )}
              </tbody>
